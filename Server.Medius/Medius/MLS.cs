...
namespace Server.Medius
{
    public class MLS : BaseMediusComponent
    {
                    ...
                    case MediusGetAllAnnouncementsRequest getAllAnnouncementsRequest:
                        {
                            ...
                            _ = Program.Database.GetLatestAnnouncements(data.ApplicationId).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                    ...
                    case MediusGetAnnouncementsRequest getAnnouncementsRequest:
                        {
                            ...
                            _ = Program.Database.GetLatestAnnouncement(data.ApplicationId).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                    ...
                    case MediusGetPolicyRequest getPolicyRequest:
                        {
                            ...
                            switch (getPolicyRequest.Policy)
                            {
                                case MediusPolicyType.Privacy:
                                    {
                                        _ = Program.Database.GetUsagePolicy().TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                                        {
                                            if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                                return;
                    ...
                    case MediusAccountGetIDRequest accountGetIdRequest:
                        {
                            ...
                            _ = Program.Database.GetAccountByName(accountGetIdRequest.AccountName, data.ClientObject.ApplicationId).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                    ...
                    case MediusAddToBuddyListRequest addToBuddyListRequest:
                        {
                            ...
                            }).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                    ...
                    case MediusGetBuddyList_ExtraInfoRequest getBuddyList_ExtraInfoRequest:
                        {
                            ...
                            /*
                            _ = Program.Database.GetAccountById(data.ClientObject.AccountId).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                    ...
                    case MediusUpdateLadderStatsWideRequest updateLadderStatsWideRequest:
                        {
                            ...
                            switch (updateLadderStatsWideRequest.LadderType)
                            {
                                ...
                                case MediusLadderType.MediusLadderTypeClan:
                                    {
                                        _ = Program.Database.PostClanLadderStats(data.ClientObject.AccountId, data.ClientObject.ClanId, pluginMessage.WideStats).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                                        {
                                            if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                                return;
                    ...
                    case MediusLadderList_ExtraInfoRequest ladderList_ExtraInfoRequest:
                        {
                            ...
                            //
                            _ = Program.Database.GetLeaderboard(ladderList_ExtraInfoRequest.LadderStatIndex + 1, (int)ladderList_ExtraInfoRequest.StartPosition - 1, (int)ladderList_ExtraInfoRequest.PageSize, data.ApplicationId).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                    ...
                    case MediusGetTotalRankingsRequest getTotalRankingsRequest:
                        {
                            ...
                            switch (getTotalRankingsRequest.LadderType)
                            {
                                case MediusLadderType.MediusLadderTypeClan:
                                    {
                                        // 
                                        _ = Program.Database.GetActiveClanCountByAppId(data.ClientObject.ApplicationId).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                                        {
                                            if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                                return;
                                ...
                                case MediusLadderType.MediusLadderTypePlayer:
                                    {
                                        ...
                                        // 
                                        _ = Program.Database.GetActiveAccountCountByAppId(data.ClientObject.ApplicationId).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                                        {
                                            if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                                return;
                    ...
                    case MediusLadderPosition_ExtraInfoRequest ladderPosition_ExtraInfoRequest:
                        {
                            ...
                            _ = Program.Database.GetPlayerLeaderboardIndex(ladderPosition_ExtraInfoRequest.AccountID, ladderPosition_ExtraInfoRequest.LadderStatIndex + 1, data.ClientObject.ApplicationId).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                    ...
                    case MediusClanLadderListRequest clanLadderListRequest:
                        {
                            ...
                            //
                            _ = Program.Database.GetClanLeaderboard(clanLadderListRequest.ClanLadderStatIndex, (int)clanLadderListRequest.StartPosition - 1, (int)clanLadderListRequest.PageSize, data.ApplicationId).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                    ...
                    case MediusPlayerInfoRequest playerInfoRequest:
                        {
                            ...
                            _ = Program.Database.GetAccountById(playerInfoRequest.AccountID).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (r.IsCompletedSuccessfully && r.Result != null)
                                {
                                    byte[] mediusStats = new byte[Constants.ACCOUNTSTATS_MAXLEN];
                    ...
                    case MediusCreateClanRequest createClanRequest:
                        {
                            ...
                            _ = Program.Database.CreateClan(data.ClientObject.AccountId, createClanRequest.ClanName, data.ClientObject.ApplicationId, Convert.ToBase64String(new byte[Constants.CLANSTATS_MAXLEN])).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (r.IsCompletedSuccessfully && r.Result != null)
                                {
                    ...
                    case MediusCheckMyClanInvitationsRequest checkMyClanInvitationsRequest:
                        {
                            ...
                            _ = Program.Database.GetClanInvitationsByAccount(data.ClientObject.AccountId).TimeoutAfter(_defaultTimeout).ContinueWith(r =>
                            {
                    ...
                    case MediusRemovePlayerFromClanRequest removePlayerFromClanRequest:
                        {
                            ...
                            {
                                _ = Program.Database.ClanLeave(data.ClientObject.AccountId, removePlayerFromClanRequest.ClanID, removePlayerFromClanRequest.PlayerAccountID).TimeoutAfter(_defaultTimeout).ContinueWith(r =>
                                {
                                    if (r.IsCompletedSuccessfully && r.Result)
                                    {
                                        data.ClientObject.Queue(new MediusRemovePlayerFromClanResponse()
                    ...
                    case MediusGetClanByNameRequest getClanByNameRequest:
                        {
                            ...
                            _ = Program.Database.GetClanByName(getClanByNameRequest.ClanName, data.ClientObject.ApplicationId).TimeoutAfter(_defaultTimeout).ContinueWith(r =>
                            {
                                if (r.IsCompletedSuccessfully && r.Result != null)
                                {
                    ...
                    case MediusClanLadderPositionRequest getClanLadderPositionRequest:
                        {
                            ...
                            _ = Program.Database.GetClanLeaderboardIndex(getClanLadderPositionRequest.ClanID, getClanLadderPositionRequest.ClanLadderStatIndex, data.ClientObject.ApplicationId).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                    ...
                    case MediusUpdateClanStatsRequest updateClanStatsRequest:
                        {
                            ...
                            // Send to Plugins
                            await Program.Plugins.OnEvent(PluginEvent.MEDIUS_CLAN_ON_UPDATE_STATS, new OnUpdateClanStatsRequestArgs() { Player = data.ClientObject, Request = updateClanStatsRequest });
                    ...
                    case MediusInvitePlayerToClanRequest invitePlayerToClanRequest:
                        {
                            ...
                            _ = Program.Database.CreateClanInvitation(data.ClientObject.AccountId, data.ClientObject.ClanId.Value, invitePlayerToClanRequest.PlayerAccountID, invitePlayerToClanRequest.InviteMessage).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                    ...
                    case MediusRespondToClanInvitationRequest respondToClanInvitationRequest:
                        {
                            ...
                            _ = Program.Database.RespondToClanInvitation(data.ClientObject.AccountId, respondToClanInvitationRequest.ClanInvitationID, respondToClanInvitationRequest.Message, (int)respondToClanInvitationRequest.Response).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                    ...
                    case MediusRevokeClanInvitationRequest revokeClanInvitationRequest:
                        {
                            ...
                            _ = Program.Database.RevokeClanInvitation(data.ClientObject.AccountId, data.ClientObject.ClanId.Value, revokeClanInvitationRequest.PlayerAccountID).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                    ...
                    case MediusDisbandClanRequest disbandClanRequest:
                        {
                            ...
                            _ = Program.Database.DeleteClan(data.ClientObject.AccountId, disbandClanRequest.ClanID).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                    ...
                    case MediusGetMyClanMessagesRequest getMyClanMessagesRequest:
                        {
                            ...
                            _ = Program.Database.GetClanMessages(data.ClientObject.AccountId, data.ClientObject.ClanId.Value, 0, 1).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                    ...
                    case MediusGetAllClanMessagesRequest getAllClanMessagesRequest:
                        {
                            ...
                            _ = Program.Database.GetClanMessages(data.ClientObject.AccountId, data.ClientObject.ClanId.Value, getAllClanMessagesRequest.Start - 1, getAllClanMessagesRequest.PageSize).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                    ...
                    case MediusSendClanMessageRequest sendClanMessageRequest:
                        {
                            ...
                            _ = Program.Database.ClanAddMessage(data.ClientObject.AccountId, data.ClientObject.ClanId.Value, sendClanMessageRequest.Message).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                    ...
                    case MediusModifyClanMessageRequest modifyClanMessageRequest:
                        {
                            ...
                            _ = Program.Database.ClanEditMessage(data.ClientObject.AccountId, data.ClientObject.ClanId.Value, modifyClanMessageRequest.ClanMessageID, modifyClanMessageRequest.NewMessage).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                    ...
                    case MediusTransferClanLeadershipRequest transferClanLeadershipRequest:
                        {
                            ...
                            _ = Program.Database.ClanTransferLeadership(data.ClientObject.AccountId, data.ClientObject.ClanId.Value, transferClanLeadershipRequest.NewLeaderAccountID).TimeoutAfter(_defaultTimeout).ContinueWith((r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                    ...
                    case MediusChatToggleRequest chatToggleRequest:
                        {
                            ...
                            // ERROR -- Need to be logged in
                            if (!data.ClientObject.IsLoggedIn)
                                throw new InvalidOperationException($"INVALID OPERATION: {clientChannel} sent {chatToggleRequest} without a being logged in.");
        ...
        {
            var response = request?.GetDefaultFailedResponse(request);
            if (response != null && response is BaseScertMessage responseMsg)
            {
                client?.Queue(responseMsg);
            }
        }
