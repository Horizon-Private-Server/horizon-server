        ...
        protected override async Task ProcessMessage(BaseScertMessage message, IChannel clientChannel, ChannelData data)
        {
            ...
            switch (message)
            {
                ...
                case RT_MSG_CLIENT_APP_TO_PLUGIN clientAppToPlugin:
                    {
                        ProcessMediusMessage(clientAppToPlugin.Message, clientChannel, data);
                        break;
                    }
                ...
                case MediusSetLocalizationParamsRequest1 setLocalizationParamsRequest:
                    {
                        ...
                        data.ClientObject.Queue(new MediusSetLocalizationParamsStatusResponse1()
                        {
                            MessageID = setLocalizationParamsRequest.MessageID,
                            StatusCode = MediusCallbackStatus.MediusSuccess
                        });
                ...
                case MediusAccountLoginRequest accountLoginRequest:
                    {
                        ...
                        {
                            Program.Database.GetAccountByName(accountLoginRequest.Username, data.ClientObject.ApplicationId).ContinueWith(async (r) =>
                            {
                ...
                case MediusTicketLoginRequest ticketLoginRequest:
                    {
                        ...
                        {
                            Program.Database.GetAccountByName(ticketLoginRequest.AccountName, data.ClientObject.ApplicationId).ContinueWith(async (r) =>
                            {
                                if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                    return;
                                ...
                                else
                                {
                                    ...
                                    {
                                        MessageID = ticketLoginRequest.MessageID,
                                        StatusCodeTicketLogin = MediusCallbackStatus.MediusAccountNotFound,
                                    });
                                }
                            });
                        }
                ...
                case MediusGetAllAnnouncementsRequest getAllAnnouncementsRequest:
                    {
                        ...
                        Program.Database.GetLatestAnnouncements(data.ClientObject.ApplicationId).ContinueWith((r) =>
                        {
                            if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                return;
                ...
                case MediusGetAnnouncementsRequest getAnnouncementsRequest:
                    {
                        ...
                        Program.Database.GetLatestAnnouncement(data.ClientObject.ApplicationId).ContinueWith((r) =>
                        {
                            if (data == null || data.ClientObject == null || !data.ClientObject.IsConnected)
                                return;
